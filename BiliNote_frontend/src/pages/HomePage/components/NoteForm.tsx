...
const formSchema = z
  .object({
    ...
    video_interval: z.coerce.number().min(0).max(30).default(0).optional(),  // 0 = 自动
    grid_size: z
      .tuple([z.coerce.number().min(0).max(10), z.coerce.number().min(0).max(10)])
      .default([0, 0])  // [0, 0] = 自动
      .optional(),
  })
  .superRefine(({ video_url, platform }, ctx) => {
    // 检测是否为本地路径（用于重新生成场景，本地已有缓存文件）
    ...
    if (platform === 'local') {
      ...
      else if (isLocalPath(video_url)) {
        // 本地路径（重新生成场景），跳过 URL 验证
      }
      else {
        try {
...
const NoteForm = () => {
  ...
  const form = useForm<NoteFormValues>({
    ...
    defaultValues: {
      ...
      style: 'minimal',
      video_interval: 0,  // 0 = 自动根据视频时长优化
      grid_size: [0, 0],  // [0, 0] = 自动根据视频时长优化
      format: [],
    },
  })
  ...
  }, [currentTask, currentTaskId, form])  // 移除 modelList 依赖，避免选择模型时触发 reset
              ...
              <FormItem className="flex-1">
                {platform === 'local' && (
                  <>
                    ...
                    >
                      {isUploading ? (
                        <p className="text-center text-sm text-emerald-500">上传中，请稍候…</p>
                      ) : uploadSuccess ? (
                  ...
                  <FormItem>
                    ...
                    <div className="flex items-center space-x-2">
                      <Input
                        ...
                        onChange={e => field.onChange([+e.target.value, field.value?.[1] || 0])}
                        className="w-16"
                      />
                      <span>x</span>
                      <Input
                        ...
                        onChange={e => field.onChange([field.value?.[0] || 0, +e.target.value])}
                        className="w-16"
                      />
                    </div>
                    <FormMessage />
            ...
            <Alert
              ...
              message={
                <div>
                  <strong>💡 提示：</strong>
                  <p>• 视频理解功能需要多模态模型</p>
                  <p>• 采样间隔和拼图尺寸设为 <strong>0</strong> 时，系统将根据视频时长自动优化参数</p>
                </div>
              }
              className="text-sm"
            />
