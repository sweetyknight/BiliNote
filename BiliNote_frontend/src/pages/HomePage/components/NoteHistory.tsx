...
const NoteHistory: FC<NoteHistoryProps> = ({ onSelect, selectedId }) => {
  ...
  const handleDeleteClick = (e: React.MouseEvent, taskId: string, taskTitle: string) => {
    e.stopPropagation()
            ...
            <div className={'mt-2 flex items-center justify-between text-[10px]'}>
              ...
              <div>
                <Button
                  ...
                  size="icon"
                  variant="ghost"
                  onClick={e => handleDeleteClick(e, task.id, task.audioMeta.title || '未命名笔记')}
                  className="h-7 w-7 shrink-0 rounded-md transition-colors hover:bg-emerald-100"
                >
                  <Trash className="h-4 w-4 text-neutral-400 transition-colors" />
                </Button>
              </div>
            </div>
          </div>
      ...
      <Dialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
        <DialogContent showCloseButton={false} className="max-w-sm">
          <DialogHeader>
            ...
            <DialogDescription>
              确定要删除笔记「{taskToDelete?.title}」吗？此操作无法撤销。
            </DialogDescription>
          </DialogHeader>
