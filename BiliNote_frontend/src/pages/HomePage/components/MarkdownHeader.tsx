...
import { Copy, Download, BrainCircuit, FileCode } from 'lucide-react'
      ...
      <div className="flex items-center gap-1">
        ...
        {onDownloadHtml && (
          <TooltipProvider>
            <Tooltip>
              <TooltipTrigger asChild>
                <Button onClick={onDownloadHtml} variant="ghost" size="sm" className="h-8 px-2">
                  <FileCode className="mr-1.5 h-4 w-4" />
                  <span className="text-sm">导出 HTML</span>
                </Button>
              </TooltipTrigger>
              <TooltipContent>下载为带样式的 HTML 文件（与预览效果一致）</TooltipContent>
            </Tooltip>
          </TooltipProvider>
        )}
        <TooltipProvider>
